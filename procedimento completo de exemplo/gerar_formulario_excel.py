...
def write_label_input(ws, row, label, hint="", input_col="D", last_input_col="H", height=28, default_val=None):
...
def write_label_textarea(ws, row, label, hint="", num_rows=4, last_col="H", default_val=None):
...
def build_protocolo(wb):
    ...
    row = write_label_textarea(ws, row, "Título da Revisão *", 
        ...
        num_rows=2,
        default_val="Métodos e Aplicações de Inferência Causal e Descoberta Causal na Literatura Científica Recente (2020-2026): Uma Revisão Sistemática")
    row = write_spacer(ws, row)
    row = write_label_input(ws, row, "Pesquisador Principal *", default_val="Eduardo Matheus Figueira")
    row = write_label_input(ws, row, "E-mail de contato", default_val="eduardo@exemplo.com")
    row = write_label_input(ws, row, "Instituição / Programa", hint="Ex.: PPGES - UFES", default_val="Universidade de Exemplo")
    row = write_label_input(ws, row, "Data do Protocolo", hint="DD/MM/AAAA", default_val="27/06/2026")
    row = write_label_input(ws, row, "Registro PROSPERO", hint="Ex.: CRD42024000000. Deixe em branco se não registrado.", default_val="CRD42026_EXEMPLO")
    ...
    for i in range(1, 9):
        if i == 1:
            vals = [str(i), "Eduardo Matheus Figueira", "Revisor", "Universidade de Exemplo", "eduardo@exemplo.com"]
        elif i == 2:
            vals = [str(i), "Revisor Exemplo", "Revisor", "Universidade de Exemplo", "revisor@exemplo.com"]
        else:
            vals = [str(i), "", "", "", ""]
...
def build_pico(wb):
    ...
    for letter, title, color_key, bg_key, hint_text in pico_items:
        ...
        default_pico_val = ""
        if letter == "P":
            default_pico_val = "Estudos e aplicações na literatura científica recente (2020-2026) contendo dados observacionais ou experimentais."
        elif letter == "I":
            default_pico_val = "Métodos de descoberta causal (aprendizado de estruturas/grafos causais) e métodos de inferência causal (estimativa de efeitos de intervenções)."
        elif letter == "C":
            default_pico_val = "Desenvolvimentos conceituais, algoritmos computacionais e domínios aplicados na ciência de dados."
        elif letter == "O":
            default_pico_val = "Avanços relatados, métricas de performance de algoritmos e impacto das aplicações práticas."
        row = write_label_textarea(ws, row, "", num_rows=3, default_val=default_pico_val)
        row = write_spacer(ws, row)
    ...
    row = write_label_textarea(ws, row, "Pergunta completa *",
        ...
        num_rows=4,
        default_val="Quais são os principais métodos e aplicações de inferência e descoberta causal reportados na literatura científica recente (2020-2026)?")
...
def build_elegibilidade(wb):
    ...
    row = write_numbered_list(ws, row, "Inclusão *", [
        "Artigos publicados em periódicos revisados por pares ou anais de conferências relevantes",
        "Estudos que abordem descoberta causal e inferência causal conjuntamente",
    ...
    row = write_numbered_list(ws, row, "Exclusão *", [
        "Artigos de opinião, editoriais, cartas ao editor ou tutoriais secundários",
        "Estudos sem texto completo disponível na internet",
        "",
        "",
    ])
    ...
    for i, st in enumerate(study_types, 1):
        default_st = "Sim" if st in ["Ensaio clínico randomizado (ECR)", "Estudo de coorte", "Quasi-experimental"] else "Não"
        row = write_table_row(ws, row, [str(i), st, default_st], col_start="B", editable=True, zebra=i % 2 == 0)
    ...
    for i, lang in enumerate(languages, 1):
        default = "Sim" if lang in ["Português", "Inglês", "Espanhol"] else "Não"
        row = write_table_row(ws, row, [str(i), lang, default], col_start="B", editable=True, zebra=i % 2 == 0)
        yn_dv2.add(ws[f"D{row-1}"])
    
    # Temporal
    row = write_spacer(ws, row, 15)
    row = write_label_input(ws, row, "Ano inicial", hint="Ex.: 2000", default_val=2020)
    row = write_label_input(ws, row, "Ano final", hint="Ex.: 2026", default_val=2026)
...
def build_estrategia_busca(wb):
    ...
    ws[f"C{row}"].font = FONT_HINT
    row += 1
    row = write_numbered_list(ws, row, "Termos do Bloco 1 *", ["inferência causal", "causal inference", "", ""])
    ...
    ws[f"C{row}"].font = FONT_HINT
    row += 1
    row = write_numbered_list(ws, row, "Termos do Bloco 2 *", ["descoberta causal", "causal discovery", "", ""])
    ...
    row = write_label_textarea(ws, row, "Expressão final *",
        ...
        num_rows=4,
        default_val='("inferência causal" OR "causal inference") AND ("descoberta causal" OR "causal discovery")')
...
def build_qualidade(wb):
    ...
    row = write_label_input(ws, row, "Ferramenta de viés *", default_val="ROBINS-I (Não randomizados)")
    ...
    row = write_label_input(ws, row, "Classificação", default_val="Alto/Moderado/Baixo risco")
    ...
    row = write_label_textarea(ws, row, "Observações",
        ...
        default_val="Utilizar ROBINS-I para avaliar os estudos observacionais que estimam o efeito causal de intervenções estruturais ou de saúde pública.")
...
def build_sintese(wb):
    ...
    for i, st in enumerate(synthesis_types, 1):
        default_plan = "Sim" if st in ["Síntese narrativa estruturada", "Análise bibliométrica"] else "Não"
        row = write_table_row(ws, row, [str(i), st, default_plan], col_start="B", editable=True, zebra=i % 2 == 0)
    ...
    row = write_label_input(ws, row, "Software de análise", default_val="Python (pandas + statsmodels)")
    sw_dv = DataValidation(type="list",
    ...
    row = write_label_textarea(ws, row, "Análises adicionais",
        ...
        default_val="Mapeamento sistemático de algoritmos por classe (baseados em restrição, baseados em score, híbridos e baseados em gradiente); Análise de coocorrência de termos de palavras-chave.")
